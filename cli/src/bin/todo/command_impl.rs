use std::collections::HashSet;

use crate::{args::ClearCommand, utils::DONE_STYLE};
use yesser_todo_db::{Task, get_index};

use crate::{args::TasksCommand, command_error::CommandError};

pub(crate) fn handle_add(command: &TasksCommand, data: &mut Vec<Task>) -> Result<(), CommandError> {
    if command.tasks.is_empty() {
        return Err(CommandError::NoTasksSpecified);
    }

    let mut seen = HashSet::new();
    for task in &command.tasks {
        if !seen.insert(task.as_str()) {
            return Err(CommandError::DuplicateInput { name: task.clone() });
        } else if data.iter().any(|x| x.name == task.as_str()) {
            return Err(CommandError::TaskExists { name: task.clone() });
        }
    }

    for task in &command.tasks {
        let task_obj: Task = Task {
            name: task.clone(),
            done: false,
        };
        data.push(task_obj);
    }
    Ok(())
}

pub(crate) fn handle_remove(command: &TasksCommand, data: &mut Vec<Task>) -> Result<(), CommandError> {
    if command.tasks.is_empty() {
        return Err(CommandError::NoTasksSpecified);
    }

    let mut seen = HashSet::new();
    for task in &command.tasks {
        if !seen.insert(task.as_str()) {
            return Err(CommandError::DuplicateInput { name: task.clone() });
        } else if get_index(data, task).is_none() {
            return Err(CommandError::TaskNotFound { name: task.clone() });
        }
    }

    for task in &command.tasks {
        if let Some(index) = get_index(data, task) {
            data.remove(index);
        }
    }
    Ok(())
}

pub(crate) fn handle_list(data: &[Task]) -> Result<(), CommandError> {
    println!("\nCurrent tasks:");
    for task in data {
        if task.done {
            println!("{}", DONE_STYLE.apply_to(task.name.as_str()));
        } else {
            println!("{}", task.name);
        }
    }

    Ok(())
}

pub(crate) fn handle_done_undone(command: &TasksCommand, data: &mut Vec<Task>, done: bool) -> Result<(), CommandError> {
    if command.tasks.is_empty() {
        return Err(CommandError::NoTasksSpecified);
    }

    let mut seen = HashSet::new();
    for task in &command.tasks {
        if !seen.insert(task.as_str()) {
            return Err(CommandError::DuplicateInput { name: task.clone() });
        } else if get_index(data, task).is_none() {
            return Err(CommandError::TaskNotFound { name: task.clone() });
        }
    }

    for task in &command.tasks {
        if let Some(index) = get_index(data, task) {
            data[index].done = done;
        }
    }

    Ok(())
}

pub(crate) fn handle_clear(command: &ClearCommand, data: &mut Vec<Task>) -> Result<(), CommandError> {
    if command.done {
        data.retain(|t| !t.done);
    } else {
        data.clear();
    }
    Ok(())
}

#[deprecated]
pub(crate) fn handle_clear_done(data: &mut Vec<Task>) -> Result<(), CommandError> {
    println!("clear-done is deprecated. Use clear -d instead.");
    handle_clear(&ClearCommand { done: true }, data)
}























































































































    println!("clear-done is deprecated. Use clear -d instead.");
    handle_clear(&ClearCommand { done: true }, data)
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_handle_add_success() {
        let mut data = vec![];
        let cmd = TasksCommand {
            tasks: vec!["task1".to_string(), "task2".to_string()],
        };
        let result = handle_add(&cmd, &mut data);
        assert!(result.is_ok());
        assert_eq!(data.len(), 2);
        assert_eq!(data[0].name, "task1");
        assert_eq!(data[1].name, "task2");
        assert!(!data[0].done);
        assert!(!data[1].done);
    }

    #[test]
    fn test_handle_add_no_tasks() {
        let mut data = vec![];
        let cmd = TasksCommand { tasks: vec![] };
        let result = handle_add(&cmd, &mut data);
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CommandError::NoTasksSpecified));
    }

    #[test]
    fn test_handle_add_duplicate_input() {
        let mut data = vec![];
        let cmd = TasksCommand {
            tasks: vec!["task1".to_string(), "task1".to_string()],
        };
        let result = handle_add(&cmd, &mut data);
        assert!(result.is_err());
        match result.unwrap_err() {
            CommandError::DuplicateInput { name } => assert_eq!(name, "task1"),
            _ => panic!("expected DuplicateInput error"),
        }
    }

    #[test]
    fn test_handle_add_task_exists() {
        let mut data = vec![Task { name: "existing".to_string(), done: false }];
        let cmd = TasksCommand {
            tasks: vec!["existing".to_string()],
        };
        let result = handle_add(&cmd, &mut data);
        assert!(result.is_err());
        match result.unwrap_err() {
            CommandError::TaskExists { name } => assert_eq!(name, "existing"),
            _ => panic!("expected TaskExists error"),
        }
    }

    #[test]
    fn test_handle_remove_success() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: false },
            Task { name: "task3".to_string(), done: false },
        ];
        let cmd = TasksCommand {
            tasks: vec!["task2".to_string()],
        };
        let result = handle_remove(&cmd, &mut data);
        assert!(result.is_ok());
        assert_eq!(data.len(), 2);
        assert_eq!(data[0].name, "task1");
        assert_eq!(data[1].name, "task3");
    }

    #[test]
    fn test_handle_remove_no_tasks() {
        let mut data = vec![];
        let cmd = TasksCommand { tasks: vec![] };
        let result = handle_remove(&cmd, &mut data);
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CommandError::NoTasksSpecified));
    }

    #[test]
    fn test_handle_remove_task_not_found() {
        let mut data = vec![Task { name: "task1".to_string(), done: false }];
        let cmd = TasksCommand {
            tasks: vec!["nonexistent".to_string()],
        };
        let result = handle_remove(&cmd, &mut data);
        assert!(result.is_err());
        match result.unwrap_err() {
            CommandError::TaskNotFound { name } => assert_eq!(name, "nonexistent"),
            _ => panic!("expected TaskNotFound error"),
        }
    }

    #[test]
    fn test_handle_remove_multiple() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: false },
            Task { name: "task3".to_string(), done: false },
        ];
        let cmd = TasksCommand {
            tasks: vec!["task1".to_string(), "task3".to_string()],
        };
        let result = handle_remove(&cmd, &mut data);
        assert!(result.is_ok());
        assert_eq!(data.len(), 1);
        assert_eq!(data[0].name, "task2");
    }

    #[test]
    fn test_handle_list() {
        let data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: true },
        ];
        let result = handle_list(&data);
        assert!(result.is_ok());
    }

    #[test]
    fn test_handle_list_empty() {
        let data = vec![];
        let result = handle_list(&data);
        assert!(result.is_ok());
    }

    #[test]
    fn test_handle_done_undone_mark_done() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: false },
        ];
        let cmd = TasksCommand {
            tasks: vec!["task1".to_string()],
        };
        let result = handle_done_undone(&cmd, &mut data, true);
        assert!(result.is_ok());
        assert!(data[0].done);
        assert!(!data[1].done);
    }

    #[test]
    fn test_handle_done_undone_mark_undone() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: true },
            Task { name: "task2".to_string(), done: true },
        ];
        let cmd = TasksCommand {
            tasks: vec!["task1".to_string()],
        };
        let result = handle_done_undone(&cmd, &mut data, false);
        assert!(result.is_ok());
        assert!(!data[0].done);
        assert!(data[1].done);
    }

    #[test]
    fn test_handle_done_undone_no_tasks() {
        let mut data = vec![];
        let cmd = TasksCommand { tasks: vec![] };
        let result = handle_done_undone(&cmd, &mut data, true);
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), CommandError::NoTasksSpecified));
    }

    #[test]
    fn test_handle_done_undone_task_not_found() {
        let mut data = vec![Task { name: "task1".to_string(), done: false }];
        let cmd = TasksCommand {
            tasks: vec!["nonexistent".to_string()],
        };
        let result = handle_done_undone(&cmd, &mut data, true);
        assert!(result.is_err());
        match result.unwrap_err() {
            CommandError::TaskNotFound { name } => assert_eq!(name, "nonexistent"),
            _ => panic!("expected TaskNotFound error"),
        }
    }

    #[test]
    fn test_handle_clear_all() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: true },
        ];
        let cmd = ClearCommand { done: false };
        let result = handle_clear(&cmd, &mut data);
        assert!(result.is_ok());
        assert!(data.is_empty());
    }

    #[test]
    fn test_handle_clear_done_only() {
        let mut data = vec![
            Task { name: "task1".to_string(), done: false },
            Task { name: "task2".to_string(), done: true },
            Task { name: "task3".to_string(), done: false },
        ];
        let cmd = ClearCommand { done: true };
        let result = handle_clear(&cmd, &mut data);
        assert!(result.is_ok());
        assert_eq!(data.len(), 2);
        assert_eq!(data[0].name, "task1");
        assert_eq!(data[1].name, "task3");
    }

    #[test]
    fn test_handle_clear_done_function() {
        let mut data = vec![
            Task { name: "done-task".to_string(), done: true },
            Task { name: "keep-task".to_string(), done: false },
        ];
        #[allow(deprecated)]
        let result = handle_clear_done(&mut data);
        assert!(result.is_ok());
        assert_eq!(data.len(), 1);
        assert_eq!(data[0].name, "keep-task");
    }
}